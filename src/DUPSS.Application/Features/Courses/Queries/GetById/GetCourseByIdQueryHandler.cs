...
{
    public class GetCourseByIdQueryHandler(IUnitOfWork unitOfWork, IMapper mapper) : ICommandHandler<GetCourseByIdQuery, GetCourseResponse>
    {

        public async Task<Result<GetCourseResponse>> Handle(GetCourseByIdQuery request, CancellationToken cancellationToken)
        {
