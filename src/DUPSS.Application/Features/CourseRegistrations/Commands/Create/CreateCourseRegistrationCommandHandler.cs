...
{
    public class CreateCourseRegistrationCommandHandler(IUnitOfWork unitOfWork) : ICommandHandler<CreateCourseRegistrationCommand>
    {
        public async Task<Result> Handle(CreateCourseRegistrationCommand request, CancellationToken cancellationToken)
        {
            // Check if course exists
            var course = await unitOfWork.Repository<Course>().GetByIdAsync(request.CourseId);
            if (course == null)
                return Result.Failure(new Error("CourseNotFound", "The specified course was not found."));

            // Check if registration already exists
            var existingRegistrations = unitOfWork.Repository<CourseRegistration>().GetQueryable()
            ...
            if (await existingRegistrations.AnyAsync(cancellationToken))
                return Result.Failure(new Error("DuplicateRegistration", "Student is already registered for this course."));
