...
{
	public class LoginHandler(UserManager<AppUser> userManager, SignInManager<AppUser> signInManager, ITokenService tokenService) : ICommandHandler<LoginCommand, LoginResponse>
	{
		public async Task<Result<LoginResponse>> Handle(LoginCommand request, CancellationToken cancellationToken)
		{
			...
			var resultOfSignIn = await signInManager.PasswordSignInAsync(user.UserName!, request.Password, false, true);
			if (resultOfSignIn.Succeeded)
			{
				// Add local login info
				var loginInfo = new UserLoginInfo("LocalLogin", user.Id.ToString(), "Local User");
				var existingLogin = await userManager.FindByLoginAsync("LocalLogin", user.Id.ToString());
				...
				var userToken = userManager.SetAuthenticationTokenAsync(user, "LocalLogin", "RefreshToken", refreshToken);
			...
			{
				throw new InvalidOperationException("Your account is not allowed to sign in. Please contact support.");
			}
			throw new UnauthorizedAccessException("Invalid login attempt. Please check your password.");
		}
	}
}
